...
export async function getAssets(
  ...
  const requestLimit =
    hasWhitelist && !isTestEnv ? limit * OVERFETCH_MULTIPLIER : limit;

  const data = await deps.coinCap.listAssets({ limit: requestLimit, offset });
  if (!deps.whitelist) return data;
  const allowed = new Set(
    (await deps.whitelist.listAllowedSymbols()).map((s) => s.toUpperCase())
  );
  const filtered = data.filter((asset) =>
    allowed.has(asset.symbol.toUpperCase())
  );
