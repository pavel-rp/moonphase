...
- Icons: Lucide for UI controls; cryptocurrency icon set for asset logos. Line style, consistent sizing (approx 30px in cards, approx 48px in detail headers). Ensure accessible labels/alt text.
...
- Interaction: approx 10% light/dark hover adjustments; clear focus ring; disabled state uses `opacity-50 pointer-events-none`.
...
- Entry: quick border pulse or fade-in/up (approx 300ms). Long content can scroll within the bounded card.
...
- Provide immediate feedback for actions (e.g., swap to loading state within approx 100ms on AI generation).
